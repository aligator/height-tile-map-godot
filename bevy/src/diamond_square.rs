...
use crate::height_map_resource::{HeightGenerator, HeightMapConfig};
use rand::prelude::*;

/// Height map generated by the diamond square algorithm.
/// Contains corner heights for terrain generation.
#[derive(Debug, Clone, PartialEq)]
pub struct HeightMap(pub Vec<Vec<i32>>);

impl std::ops::Deref for HeightMap {
...
impl HeightGenerator for DiamondSquareGenerator {
    fn generate(&self, config: &HeightMapConfig) -> HeightMap {
        let mut rng = StdRng::seed_from_u64(config.seed);
        self.generate(config.size, config.roughness, config.max_height, &mut rng)
    }
}
