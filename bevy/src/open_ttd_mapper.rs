...
use crate::{diamond_square::HeightMap, height_map_resource::TileMapper};
...
/// Tile heights map generated by the OpenTTD mapper.
/// Contains the height of the lowest corner for each tile.
#[derive(Debug, Clone, PartialEq)]
pub struct TileHeights(pub Vec<Vec<i32>>);

impl std::ops::Deref for TileHeights {
...
impl std::fmt::Display for TileHeights {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let max_display_size = 20; // Limit display size for readability
        let display_height = self.0.len().min(max_display_size);
        let display_width = if self.0.is_empty() {
            0
        } else {
            self.0[0].len().min(max_display_size)
        };

        writeln!(
            f,
            "TileHeights ({}x{}) - lowest corner heights:",
            self.0.len(),
            if self.0.is_empty() {
                0
            } else {
        ...
        for y in 0..display_height {
            for x in 0..display_width {
                write!(f, "{:2} ", self.0[x][y])?;
            }
            if display_width < self.0[0].len() {
                write!(f, "...")?;
            }
            writeln!(f)?;
        }

        if display_height < self.0.len() {
            writeln!(f, "...")?;
        }
...
impl OpenTTDMapper {
    ...
    pub fn build(&self, corner_heights: &[Vec<i32>]) -> TileHeights {
